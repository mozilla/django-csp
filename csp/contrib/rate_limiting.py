...
class RateLimitedCSPMiddleware(CSPMiddleware):
    ...
    def get_policy_parts(self, request: HttpRequest, response: HttpResponseBase, report_only: bool = False) -> PolicyParts:
        ...
        # If CSP isn't configured in settings, return the policy parts as-is.
        ...
        if remove_report:
            # For the report-only policy we remove the entire header to avoid browser warnings about a
            # report-only policy with no reporting endpoint defined.
            if report_only:
                policy_parts.config = {}
                policy_parts.replace = None
                policy_parts.update = None
            else:
                if policy_parts.replace is None:
                    ...
                    policy_parts.replace = copy.deepcopy(policy_parts.replace)
